...
function renderScreenSpotlightCard(state: GameSessionState, t: (key: string) => string) {
  if (!state.screenSpotlight) {
    return "";
  }

  return `<div class="reward-highlight-card overlay-focus-card screen-spotlight-card"><div class="panel-actions"><span class="tag tag-accent">${t("ui.nextStep")}</span><span class="tag">${t("ui.recommended")}</span><span class="tag">${t(state.screenSpotlight.tagKey)}</span></div><strong>${t(state.screenSpotlight.titleKey)}</strong><div class="small">${t(state.screenSpotlight.bodyKey)}</div></div>`;
}
...
function renderPreLevelModal(state: GameSessionState, t: (key: string) => string) {
  ...
  return `<div class="prelevel-modal"><div class="panel-actions"><span class="tag">${t("preLevel.title")}</span><span class="tag">${t(`difficulty.${level.difficulty}`)}</span></div><h2>${t("map.play")} ${level.id}</h2><p class="small">${t("preLevel.subtitle")}</p>${renderPreLevelContextCard(state, t)}<div class="prelevel-summary-grid"><div class="metric-card"><div class="small">${t("level.goal")}</div><strong>${t(`objective.${level.objective.type}`)}${level.objective.target ? ` ${level.objective.target}` : ""}</strong></div><div class="metric-card"><div class="small">${t("level.moves")}</div><strong>${level.moves}</strong></div></div><div class="small prelevel-rewards"><strong>${t("preLevel.rewards")}</strong><br />${formatRewardSummary(level.rewards, t)}</div><div class="panel-actions"><strong>${t("preLevel.boosters")}</strong><span class="small">${t("preLevel.limit")}</span></div><div class="prelevel-booster-grid">${preLevelBoosterIds.map((boosterId) => renderPreLevelBoosterCard(state, boosterId, selectedBoosters, t)).join("")}</div><div class="cta-row"><button class="ghost-btn" data-action="close-level-preview">${t("preLevel.back")}</button><button class="primary-btn" data-action="confirm-start-level">${t("preLevel.start")}</button></div></div>`;
}

function renderPreLevelContextCard(state: GameSessionState, t: (key: string) => string) {
  if (state.mapSpotlight?.action.action !== "start-current-level") {
    return "";
  }

  const recoveryContext = state.mapSpotlight.tagKey === "reward.reveal.gemSafetyTag";
  return `<div class="reward-reveal-summary reward-highlight-card prelevel-context-card ${recoveryContext ? "is-recovery-ready" : ""}"><div class="panel-actions"><span class="tag tag-accent">${t("ui.nextStep")}</span><span class="tag">${t("ui.recommended")}</span><span class="tag ${recoveryContext ? "tag-accent" : ""}">${t(state.mapSpotlight.tagKey)}</span></div><strong>${t(state.mapSpotlight.titleKey)}</strong><div class="small">${t(state.mapSpotlight.bodyKey)}</div></div>`;
}
