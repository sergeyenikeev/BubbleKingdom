...
| Yandex SDK integration exists for Yandex build | `packages/platform-sdk/src/adapters/yandex.ts` (`/sdk.js` primary path with CDN fallback) | Typecheck/build coverage | Run Yandex sandbox build and confirm SDK boot succeeds |
...
| All IAP in Yandex build go through Yandex SDK | `packages/platform-sdk/src/adapters/yandex.ts` purchase methods, signed-receipt path for server validation | Integration mock purchase flow | Validate catalog, signed receipt callbacks, and backend validation in Yandex sandbox |
