...
- Live-event overlays now close one more conversion gap too: when a season follow-up reaches `event` before any milestone is claimable, the screen still surfaces a clear `Play next level` return beat instead of ending on context alone.
- Restoration overlays now close the same gap for shortage states: when the recommended upgrade is not yet affordable, the top CTA pivots into `Play next level` so the player gets a clear earning route instead of a passive blocked-upgrade panel.
- Real restoration reveals now keep that continuity too: `restore -> View next restore` promotes a next-landmark highlight that survives dismissal to the map and becomes a proper `screenSpotlight` after the player opens the restoration overlay.
- Live-event reward claims now keep the same continuity standard: if another milestone is already claimable, dismissing the first claim reveal preserves that next reward inside the `event` overlay and only then falls back to the map spotlight on exit.
- If an event claim leaves no immediately claimable milestone, the same reveal path now becomes a `Play next level` earning route, keeping the event loop active without pretending there is another reward ready.
- The `event` overlay now mirrors the single-focus rule more strictly by highlighting only the recommended milestone card, so multiple claimable rewards no longer create duplicate top-priority emphasis.
